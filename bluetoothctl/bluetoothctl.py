...
def force_reconnect_device(dev_mac:str,log_level=logging.WARNING)->None:
    ...
    #    
    if dev_paired:
        res=input(f"Would you like to disconnect the {dev_mac} device?")
        if(res.lower() not in ("y","yes")):
            print("Stop disconnecting %s!" % dev_mac)
            return
    ...
    if(res.lower() not in ("y","yes")):
        ...
        return
    # start scanning
    res=bt.command_scan_on(dev_mac)
    mtc=bt.get_last_match()
    # Pairing devices
    res=bt.command_pair_device(dev_mac)
    mtc=bt.get_last_match()
    print("Paired device! (%s)" % mtc)
    # Connecting devices
    res=bt.command_connect_device(dev_mac)
    mtc=bt.get_last_match()
    print("Connected device! (%s)" % mtc)
    # Stop scanning
    bt.command_scan_off()
